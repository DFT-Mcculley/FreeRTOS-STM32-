...
- 和BASEPRI：这个寄存器最多有9位(由表达优先级的位数决定)。它定义了被屏蔽优先级的阈值。当它被设成某个值后，所有优先级号大于等于此值的中断都被关(优先级号越大，优先级越低)。但若被设成0，则不关闭任何中断，0也是缺省值。（FreeRTOS采用该方式进行中断屏蔽）
...
```c
#define portDISABLE_INTERRUPTS()  vPorRaiseBASEPP();  //中断
```

![image-20230829171831271](C:\Users\dft\Desktop\FreeRTOS(STM32)\FreeRTOS⼊⻔（STM32）\FreeRTOS⼊⻔（STM32）.assets\image-20230829171831271.png) 

![image-20230829172503414](C:\Users\dft\Desktop\FreeRTOS(STM32)\FreeRTOS⼊⻔（STM32）\FreeRTOS⼊⻔（STM32）.assets\image-20230829172503414.png) 